...
Chain.prototype.blockHandler = function(block, blockBytes, add, nextBlock) {
  ...
  for (var t = 0; t < transactionsLength; t++) {
    ...
    if (!tx.isCoinbase()) {
      for (var inputIndex = 0; inputIndex < tx.inputs.length; inputIndex++) {
        ...
        if (prevOutputValue) {
          ...
          if (deltas[key]) {
            deltas[key] -= prevOutputSatoshis;
          } else {
            deltas[key] = prevOutputSatoshis * -1;
          }

          self.txn.del(self.unspentOutputsDbi, prevOutputKey);
        }
      }
    }
    ...
    for (var outputIndex = 0; outputIndex < tx.outputs.length; outputIndex++) {
      ...
      var outKey = outputAddressInfo.hashBuffer.toString('binary') + outputAddressInfo.hashTypeBuffer.toString('binary');
      if (deltas[outKey]) {
        deltas[outKey] += output.satoshis;
      } else {
        deltas[outKey] = output.satoshis;
      }
    ...
    for (var addressKey in deltas) {
      ...
      if (addressSum) {
        self.txn.del(self.addressSumDbi, addressKey);
      ...
      self.txn.putBinary(self.addressSumDbi, addressKey, value);
    }
