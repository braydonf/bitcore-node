...
function Chain(options) {
  ...
  this.enableBalanceIndex = options.enableBalanceIndex || constants.ENABLE_BALANCE_INDEX;
  if (this.enableBalanceIndex) {
    log.info('Balance index is enabled');
  } else {
    log.info('Balance index is disabled');
  }
...
Chain.prototype.start = function(callback) {
  ...
  if (self.enableBalanceIndex) {
    self.addressBalanceDbi = self.env.openDbi({
      name: 'addressBalance',
      create: true
    });
  } else {
    self.addressBalanceDbi = false;
  }
  self.outputSpentDbi = self.env.openDbi({
    name: 'outputSpent',
    create: true
  });
  ...
  for (var i = 0; i < numCPUs; i++) {
    var worker = spawn('node', [path.resolve(__dirname, './blockworker.js')], { stdio: [0, 1, 2, 'ipc']});
    self.blockWorkers.push(worker);
  }
...
Chain.prototype.stop = function(callback) {
  ...
  async.whilst(function() {
    ...
    if (self.dbOpen) {
      if (self.txn) {
        log.warn('Shutting down while txn is uncommited. Aborting transaction.');
        self.txn.abort();
      }
      self.blocksDbi.close();
      self.outputSpentDbi.close();
...
Chain.prototype._inputAddressSumHandler = function(input, add, deltas) {
  var self = this;
...
Chain.prototype._outputAddressSumHandler = function(output, addressInfo, add, deltas) {
  var outKey = addressInfo.hashBuffer.toString('binary') + addressInfo.hashTypeBuffer.toString('binary');
...
Chain.prototype._putAddressSums = function(deltas, heightBuffer) {
  var self = this;
  // Update address balances
  for (var addressKey in deltas) {
    var addressBalanceValue = self.txn.getBinary(self.addressBalanceDbi, addressKey);
    var addressBalance = new Buffer(new Array(8));

    if (addressBalanceValue) {
      var balance = addressBalanceValue.readDoubleBE() + deltas[addressKey];
      addressBalance.writeDoubleBE(balance);
    } else {
      addressBalance.writeDoubleBE(deltas[addressKey]);
    }

    self.txn.putBinary(self.addressBalanceDbi, addressKey, Buffer.concat([addressBalance, heightBuffer]));
  }
};

Chain.prototype.blockWriter = function(blockIndexes, tipHash, tipHeight, callback) {
  var self = this;

  console.log('Starting writing blocks...');
  ...
  for (var i = 0; i < blockIndexes.length; i++) {
    var addressOutputs = blockIndexes[i].addressOutputs;
    var outputSpents = blockIndexes[i].outputSpents;
    
    for(var addressKey in addressOutputs) {
      var out = addressOutputs[addressKey];
      for (var j = 0; j < out.length; j++) {
        self.txn.putString(self.addressOutputsDbi, addressKey, out[j]);
      }
    }

    for(var outKey in outputSpents) {
      self.txn.putString(self.outputSpentDbi, outKey, outputSpents[outKey]);
    }

    self.txn.putString(self.blocksDbi, blockIndexes[i].timestamp, blockIndexes[0].meta);
  }

  var blockHashBuffer = new Buffer(tipHash, 'hex');
  self.txn.putString(self.blocksDbi, constants.TIP_KEY, blockHashBuffer.toString('binary'));
  ...
  self.env.sync(function(err){
    ...
    self.txn = false;
    console.log('End writing blocks...');
    self.getBlock(tipHash, function(err, block) {
      if (err) {
        return callback(err);
      }
      block.__height = tipHeight;
      self.tip = block;
      console.log('Set new tip');
  ...
  for (var t = 0; t < transactionsLength; t++) {
    ...
    if (!tx.isCoinbase()) {
      for (var inputIndex = 0; inputIndex < tx.inputs.length; inputIndex++) {
        ...
        if (add) {
          self.txn.putBinary(self.outputSpentDbi, inputKey, inputValue);
        } else {
          self.txn.del(self.outputSpentDbi, inputKey);
        }

        if (self.enableBalanceIndex) {
          self._inputAddressSumHandler(input, add, deltas);
        }
    ...
    for (var outputIndex = 0; outputIndex < tx.outputs.length; outputIndex++) {
      ...
      var addressInfo = encoding.extractAddressInfoFromScript(outputScript, self.node.network);
      if (!addressInfo) {
        continue;
      }
      ...
      if (self.enableBalanceIndex) {
        self._outputAddressSumHandler(output, add, deltas);
      }

    }

    if (self.enableBalanceIndex) {
      self._putAddressSums(deltas);
    }
...
Chain.prototype.loadTip = function(callback) {
  ...
  var tipDataString = txn.getString(self.blocksDbi, constants.TIP_KEY);
  txn.abort();
  ...
  var tipData = new Buffer(tipDataString, 'binary');
...
Chain.prototype.sync = function() {
  ...
  async.whilst(function() {
    ...
    var BLOCK_READ_COUNT = 200;

    var newTipHeight = height + BLOCK_READ_COUNT;
    var newTipBlockIndex = self.node.services.bitcoind.getBlockIndex(newTipHeight);
    ...
    function syncMany() {
      console.log('Starting to read blocks...');
      async.timesLimit(BLOCK_READ_COUNT, 5, function(n, next) {
        var blockHeight = height + n + 1;
        self.node.services.bitcoind.getBlock(blockHeight, function(err, blockBuffer) {
          if (err) {
            return next(err);
          }
          next(null, [blockHeight, blockBuffer]);
        });
      }, function(err, blocks) {
        ...
        function readComplete() {
          for (var i = 0; i < self.blockWorkers.length; i++) {
            self.blockWorkers[i].removeListener('message', listeners[i]);
          }

          self.blockWriter(blockIndexes, newTipBlockIndex.hash, newTipHeight, function(err) {
            if (err) {
              return done(err);
            }
            done();
        ...
        function sendBlock(worker) {
          if (blockIndexes.length === BLOCK_READ_COUNT) {
            return readComplete();
          }
          var item = blocks.pop();
          if (item) {
            var height = item[0];
            var blockData = item[1];
            worker.send({block: blockData, height: height});
          }
        }

        self.blockWorkers.forEach(function(worker) {
          var workerListener = function(message) {
            blockIndexes.push(message);
            sendBlock(worker);
          };
