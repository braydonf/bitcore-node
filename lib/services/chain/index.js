...
Chain.prototype.blockHandler = function(block, blockBytes, add, nextBlock) {
  ...
  for (var t = 0; t < transactionsLength; t++) {
    ...
    if (!tx.isCoinbase()) {
      for (var inputIndex = 0; inputIndex < tx.inputs.length; inputIndex++) {
        ...
        if (prevOutputValue) {
          ...
          if (!txids[key]) {
            txids[key] = Buffer.concat([txHashBuffer, heightBuffer]);
          }
          //TODO: Mark address output as spent (or delete)
        }
      }
    }
    ...
    for (var outputIndex = 0; outputIndex < tx.outputs.length; outputIndex++) {
      ...
      if (!txids[outKey]) {
        txids[outKey] = Buffer.concat([txHashBuffer, heightBuffer]);
      }
    }
    ...
    // Update txids for addresses
    for (var addressKeyTxid in txids) {
      if (add) {
        self.txn.putBinary(self.addressTxidsDbi, addressKeyTxid, txids[addressKeyTxid]);
      } else {
        // TODO remove
      }
    }
