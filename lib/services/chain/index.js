...
Chain.prototype.start = function(callback) {
  ...
  self.outputsDbi = self.env.openDbi({
    name: 'outputs',
    create: true
  });
  self.addressTransactionsDbi = self.env.openDbi({
...
Chain.prototype.stop = function(callback) {
  ...
  async.whilst(function() {
    ...
    if (self.dbOpen) {
      ...
      self.outputsDbi.close();
      self.addressTransactionsDbi.close();
  ...
  for (var t = 0; t < transactionsLength; t++) {
    ...
    if (!tx.isCoinbase()) {
      for (var inputIndex = 0; inputIndex < tx.inputs.length; inputIndex++) {
        ...
        var prevOutputValue = self.txn.getBinary(self.outputsDbi, prevOutputKey);
        ...
        if (prevOutputValue) {
          ...
          inputIndexBuffer.writeUInt32BE(inputIndexBuffer);
          var spentBuffer = Buffer.concat([txHashBuffer, inputIndexBuffer]);

          if (add) {
            self.txn.putBinary(self.outputsDbi, prevOutputKey, Buffer.concat([prevOutputValue, spentBuffer]));
          } else {
            self.txn.putBinary(self.outputsDbi, prevOutputKey, prevOutputValue.slice(0, 29));
          }
        }
      }
    }
    ...
    for (var outputIndex = 0; outputIndex < tx.outputs.length; outputIndex++) {
      ...
      // Keep track of each output for address -> output
      var addressKey = addressInfo.hashBuffer.toString('binary') + addressInfo.hashTypeBuffer.toString('binary');
      var outputIndexBuffer = new Buffer(new Array(4));
      outputIndexBuffer.writeUInt32BE(outputIndex);
      if (addressTransactions[addressKey]) {
        addressTransactions[addressKey].push(outputIndexBuffer);
      } else {
        addressTransactions[addressKey] = [outputIndexBuffer];
      }

      // Add index for output -> address and satoshis
      var outputKey = txHashBuffer.toString('binary') + outputIndex;
      var satoshisBuffer = new Buffer(new Array(8));
      satoshisBuffer.writeDoubleBE(satoshisBuffer);
      if (add) {
        self.txn.putBinary(self.outputsDbi, outputKey, Buffer.concat([
          satoshisBuffer,
          addressInfo.hashBuffer,
          addressInfo.hashTypeBuffer
        ]));
      } else {
        self.txn.del(self.outputsDbi, outputKey);
      }
    ...
    // Put updates for address -> outputs (by txid)
    for (var addressKey in addressTransactions) {
      var outputIndexBuffers = Buffer.concat(addressTransactions[addressKey]);
      ...
      if (add) {
        self.txn.putBinary(self.addressTransactionsDbi, addressKey, outputValue);
      } else {
        self.txn.del(self.addressTransactionsDbi, addressKey);
      }
    }


    // Put updates for address -> balance
