...
DB.prototype.runAllBlockHandlers = function(block, blockBytes, add, callback) {
  ...
  operations.push({
    ...
    value: this._encodeBlockIndexValue(block.hash, block.transactions.length, blockBytes)
  });
...
DB.prototype._encodeBlockIndexValue = function(hash, transactionCount, bytes) {
  var transactionCountBuffer = new Buffer(new Array(4));
  transactionCountBuffer.writeUInt32BE(transactionCount);
  var bytesBuffer = new Buffer(new Array(8));
  bytesBuffer.writeDoubleBE(bytes);
  return Buffer.concat([new Buffer(hash, 'hex'), transactionCountBuffer, bytesBuffer]);
};

DB.prototype._decodeBlockIndexValue = function(value) {
  var blockHash = value.slice(0, 32).toString('hex');
  var transactionCount = value.readUInt32BE(32);
  var bytes = value.readUINt32BE(36);
  return {
  ...
  self.findCommonAncestor(block, function(err, ancestorHash) {
    ...
    async.whilst(
      ...
      function(removeDone) {
        ...
        var tip = self.tip;
        var tipBytes = self.tip.toBuffer().length;
