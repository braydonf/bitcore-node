...
var AddressService = function(options) {
  ...
  this.maxQueryTime = options.maxQueryTime || constants.MAX_QUERY_TIME;
...
AddressService.prototype._getAddressConfirmedInputsSummary = function(address, result, options, callback) {
  ...
  inputsStream.on('data', function(input) {
    ...
    if (new Date() - result.startTime > self.maxQueryTime) {
      error = new Error('Query has timed out');
      inputsStream.end();
    } else if (count > self.maxInputsQueryLength) {
...
AddressService.prototype._getAddressConfirmedOutputsSummary = function(address, result, options, callback) {
  ...
  outputStream.on('data', function(output) {
    ...
    if (new Date() - result.startTime > constants.maxQueryTime) {
      error = new Error('Query has timed out');
      outputStream.end();
    } else if (count > self.maxOutputsQueryLength) {
