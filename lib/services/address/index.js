...
var AddressService = function(options) {
  ...
  $.checkArgument(
    _.isUndefined(options.maxTransactions) ||
      JSUtil.isNaturalNumber(options.maxTransactions),
    'maxTransactions option is expected to be a positive integer'
  );
  this.maxTransactions = options.maxTransactions;
...
AddressService.prototype.getInputs = function(addressStr, options, callback) {
  ...
  stream.on('data', function(data) {
    ...
    if (self.maxTransactions && inputs.length > self.maxTransactions) {
      stream.pause();
      error = new Error('Maximum number of transaction per query reached for address: ' + addressStr);
      log.warn(error);
      stream.destroy();
    }
...
AddressService.prototype.getOutputs = function(addressStr, options, callback) {
  ...
  stream.on('data', function(data) {
    ...
    if (self.maxTransactions && outputs.length > self.maxTransactions) {
      stream.pause();
      error = new Error('Maximum number of transaction per query reached for address: ' + addressStr);
      log.warn(error);
      stream.destroy();
