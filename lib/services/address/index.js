...
AddressService.prototype.getAddressSummary = function(addressArg, options, callback) {
  ...
  async.waterfall([
    ...
    function(cache, next) {
      self._getAddressTransactionSummary(address, cache, tipHeight, next);
    },
    function(cache, next) {
      self._saveAddressSummaryCache(address, cache, tipHeight, next);
    },
    function(cache, next) {
      self._getAddressMempoolSummary(address, cache, tipHeight, next);
    }
  ], function(err, cache) {
    if (err) {
      return callback(err);
    ...
    var summary = {
      ...
      appearances: result.appearanceIds.size,
      unconfirmedBalance: result.unconfirmedBalance,
      unconfirmedAppearances: result.unconfirmedAppearanceIds.size
    };
    ...
    var fullTxids = result.appearanceIds;
    for (var entry of result.unconfirmedAppearanceIds.entries()) {
      fullTxids.set(entry[0]);
    }
    if (options.fullTxList) {
      summary.fullTxids = fullTxids;
    } else if (!options.noTxList) {
      summary.txids = [];
      for(var i = 0; i < fullTxids.length; i++) {
        summary.txids.push(fullTxids[i][0]);
      }
    }
...
AddressService.prototype._saveAddressSummaryCache = function(address, cache, tipHeight, callback) {
  ...
  var transactionLength = cache.result.appearanceIds.size;
...
AddressService.prototype._getAddressSummaryCache = function(address, callback) {
  var baseCache = {
    result: {
      appearanceIds: new Map(),
      totalReceived: 0,
      balance: 0,
      unconfirmedAppearanceIds: new Map(),
...
AddressService.prototype._getAddressTransactionSummary = function(address, cache, tipHeight, callback) {
  ...
  var stream = new CombinedStream({
    inputStream: self.createInputsStream(address, opts),
    outputStream: self.createOutputsStream(address, opts)
  });

  stream.on('data', function(inputsAndOutputs) {
    for (var k = 0; k < inputsAndOutputs.length; k++) {
      var value = inputsAndOutputs[k];
      var txid = value.txid;

      if (value.inputIndex) {
        // Handle the case that this is an input
        cache.result.appearanceIds.set(txid, value.height);

      } else if (value.outputIndex) {
        // Handle the case that this is an output

        var outputIndex = value.outputIndex;

        // Bitcoind's isSpent only works for confirmed transactions
        var spentDB = self.node.services.bitcoind.isSpent(txid, outputIndex);
        cache.result.totalReceived += value.satoshis;
        cache.result.appearanceIds.set(txid, value.height);

        if (!spentDB) {
          cache.result.balance += value.satoshis;
        }
        ...
        if (spentMempool) {
          cache.result.unconfirmedBalance -= value.satoshis;
        }
      }
    }
  });
...
AddressService.prototype._getAddressMempoolSummary = function(address, cache, tipHeight, callback) {
  var self = this;

  var addressStr = address.toString();
  var hashBuffer = address.hashBuffer;
  var hashTypeBuffer = constants.HASH_TYPES_BUFFER[address.type];
  ...
  self._getInputsMempool(addressStr, hashBuffer, hashTypeBuffer, function(err, mempoolInputs) {
    ...
    for(var i = 0; i < mempoolInputs.length; i++) {
      ...
      cache.result.unconfirmedAppearanceIds.set(input.txid);
    }
    ...
    self._getOutputsMempool(addressStr, hashBuffer, hashTypeBuffer, function(err, mempoolOutputs) {
      ...
      for(var i = 0; i < mempoolOutputs.length; i++) {
        ...
        cache.result.unconfirmedAppearanceIds.set(output.txid);
