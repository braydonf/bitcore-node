...
AddressService.prototype.blockHandler = function(block, addOutput, callback) {
  ...
  heightBuffer.writeUInt32BE(heightBuffer);
  ...
  for (var i = 0; i < transactionLength; i++) {
    ...
    for (var outputIndex = 0; outputIndex < outputLength; outputIndex++) {
      ...
      satoshisBuffer.writeDoubleBE(output.satoshis);
      ...
      var key = encoding.encodeOutputKey(addressInfo.hashBuffer, addressInfo.hashTypeBuffer,
                                         heightBuffer, txidBuffer, outputIndexBuffer);
      operations.push({
        type: action,
        key: key,
        value: satoshisBuffer
      });

      // For the purposes of populating previous output information on a transaction's
      // inputs, as tha can be an expensive operation. This can also be useful when parsing a block
      // to know which addresses are being spent for keeping an ongoing sum for addresses.
      var prevKey = Buffer.concat([constants.PREFIXES.PREVOUTPUTS, txidBuffer, outputIndexBuffer]);
      var prevValue = Buffer.concat([addressInfo.hashBuffer, addressInfo.hashTypeBuffer, satoshisBuffer]);
      operations.push({
        type: action,
        key: prevKey,
        value: prevValue
    ...
    for(var inputIndex = 0; inputIndex < inputs.length; inputIndex++) {
      ...
      // This index is useful for determining if an output has been spent, where it was spent
      // and at what block it was spent for determining the number of confirmations. It's possible
      // to query bitcoind for this information, however some of the information is purged.
      ...
      var inputValueMap = encoding.encodeInputValueMap(txidBuffer, inputIndex, heightBuffer);
