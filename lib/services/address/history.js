...
AddressHistory.prototype.get = function(callback) {
  ...
  options.fullTxList = true;

  this.node.services.address.getAddressSummary(address, options, function(err, summary) {
    if (err) {
      return callback(err);
    }

    totalCount = summary.fullTxids.size;

    var fromOffset = totalCount - self.options.from;
    var toOffset = totalCount - self.options.to;
    var txids = Array.from(summary.fullTxids.keys()).slice(toOffset, fromOffset);
