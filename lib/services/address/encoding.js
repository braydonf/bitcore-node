...
exports.encodeSummaryCacheValue = function(cache, tipHeight) {
  ...
  var buffers = [];
  for (var entry of cache.result.appearanceIds.entries()) {
    var txidBuffer = new Buffer(entry[0], 'hex');
    var heightBuffer = new Buffer(new Array(4));
    heightBuffer.writeUInt32BE(entry[1]);
    buffers.push(Buffer.concat([txidBuffer, heightBuffer]));
  }
  var txidsBuffer = Buffer.concat(buffers);
...
exports.decodeSummaryCacheValue = function(buffer) {
  ...
  var appearanceIds = new Map();
  var pos = 20;
  while(pos < buffer.length) {
    var offset = pos + 32;
    var txid = buffer.slice(pos, offset).toString('hex');
    var txidHeight = buffer.readUInt32BE(offset);
    appearanceIds.set(txid, txidHeight);
    pos += 36;
  }
  ...
  var cache = {
    ...
    result: {
      ...
      balance: balance,
      unconfirmedAppearanceIds: new Map(), // unconfirmed values are never stored in cache
...
exports.getAddressInfo = function(addressStr) {
  ...
  var hashTypes = Object.keys(constants.HASH_TYPES_READABLE);
  var hashTypeBuffer;
  for (var i = 0; i < hashTypes.length; i++) {
    if (constants.HASH_TYPES_READABLE[hashTypes[i]] === addrObj.type) {
