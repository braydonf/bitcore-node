...
function registerSyncHandlers(node, delay) {
  ...
  delay = delay || 1000 * 60 * 5;
  var interval = false;
  var lastHeight = 0;
  var lastHeightDate = new Date();

  function logSyncStatus() {
    var heightDelta = node.services.chain.tip.__height - lastHeight;
    var timeDelta = (new Date() - lastHeightDate) / 1000;
    log.info(
      'Database Sync Status: Tip:', node.services.chain.tip.hash,
      'Height:', node.services.chain.tip.__height,
      'Rate:', (heightDelta / timeDelta).toFixed(2), 'blocks per second'
    );
    lastHeight = node.services.chain.tip.__height;
    lastHeightDate = new Date();
  }

  node.on('ready', function() {
    lastHeight = node.services.chain.tip.__height;
    if (node.services.chain) {
      node.on('synced', function() {
        clearInterval(interval);
        logSyncStatus();
      });
      node.services.chain.on('addblock', function(block) {
